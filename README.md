...
| **프로필** | 계열사·선호 지역·나이·관심사·취미와 매칭 성향. 바꾸면 매칭 정렬에 바로 반영됩니다 |
...
| 장소 검색 | 카카오 로컬 키워드 검색(REST · 서버 전용 키), 실패 시 OpenRouter 웹 검색 플러그인으로 대체 |
| 지도 | 카카오맵 JavaScript SDK (공개 JS 키 · 도메인 제한) |
| 테스트 | Node.js 24 내장 test runner — `npm test` |

`src/js`는 `config` → `app` → `home`/`match`/`chat`/`profile` → `map` → `backend` → `boot` 순서로 로드되며, 전역 스코프를 공유합니다. Edge Function의 `_shared/` 모듈은 `Deno` 전역을 쓰지 않는 순수 모듈이라 서버 로직을 Node 테스트에서 그대로 검증합니다.
...
- **익명화** — 프롬프트에는 모임 요약과 익명 처리된 최근 대화만 들어갑니다. 실명·사번·사용자 ID는 넣지 않습니다 (`_shared/chat.ts`의 `anonymizeMessages`).
- **환각 차단** — LLM이 제안한 장소 이름을 그대로 쓰지 않습니다. `verifyPlan`이 카카오 장소 검색 결과와 대조해 **검색에 없는 장소는 후보에서 빼고**, 남은 후보는 실제 장소 ID·주소·좌표·상세 링크로 덮어씁니다. 실재하는 장소만 지도에 올라갑니다.
- **실패 대비** — 요청당 20초 타임아웃, 429·5xx 재시도, `response_format`을 거부하는 모델을 위한 compat 모드, 그래도 실패하면 정적 `fallbackPlan`. 장소 검색이 실패해도 약속 추천은 진행되고, 왜 후보가 비었는지를 `quota` · `auth` · `error` · `empty` · `no_key`로 구분해 화면에 알립니다.
