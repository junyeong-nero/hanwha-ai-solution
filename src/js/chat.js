...
async function unvote(i){
  const id=CUR, r=S.rooms[id], msg=r&&r.msgs[i]; if(!msg||!msg.planId)return;
    ...
    if(m.f==='ai'){
      ...
      const cands=(p.cands||[]).length
        ? '<div class="cands"><b>후보지 · 실제 장소 검색 결과 '+p.cands.length+'곳</b>'
          +planMapHtml(m.planId,p.cands)+candListHtml(m.planId,p.cands,done||stale)+note+'</div>'
      ...
      const btn=stale
        ...
        :'<button '+(done||mine?'disabled':'')+' onclick="confirmPlan('+i+')">'+(auto?'시간이 지나 확정됨':done?'약속 확정됨':mine?'확정했어요 ✓ · 다른 멤버 기다리는 중':'이 약속으로 확정')+'</button>'
          +(mine&&!done?'<button class="unvote" onclick="unvote('+i+')">확정 취소</button>':'');
...
async function aiPlan(){
  ...
  setTimeout(()=>{
    ...
    const msg={f:'ai',plan:Object.assign({},base,{cands,meetAt:new Date(Date.now()+(base.inH||0)*3600000).toISOString()}),planId:'local-'+id+(seq>1?'-'+seq:''),t:nowT(),
