...
test('홈은 다음 행동 하나만 표시하고 모두 완료하면 사라진다', () => {
  const app = loadApp();
  app.evaluate('S.profile.interests=[];S.profile.hobbies=[];S.joined=[];S.placeRecommendationTried=false');
  const states = [
    ['', '관심사·취미 설정', "go('profile')"],
    ["S.profile.interests=['독서']", '어울리는 모임 참가', "go('match')"],
    ["S.joined=['m1']", 'AI 장소 추천 기능 써보기', 'tryPlaceRecommendations()'],
  ];
  for (const [setup, title, action] of states) {
    app.evaluate(setup+';renderHome()');
    const card = app.el('nextcard').innerHTML;
    assert.ok(card.includes('<p class="next-action">'+title+'</p>'));
    assert.ok(card.includes('onclick="'+action+'"'));
    assert.equal((card.match(/<button /g)||[]).length, 1);
    assert.doesNotMatch(card, /<details|class="ns/);
  }
