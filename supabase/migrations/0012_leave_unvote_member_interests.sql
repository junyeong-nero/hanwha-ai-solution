-- 0012 — 모임 나가기 · 확정 투표 취소 · 멤버 관심사 태그
...
begin
  ...
  if exists (
    ...
    raise exception 'ATTENDED' using errcode = 'P0001', message = '만남을 완료한 모임은 나갈 수 없어요';
  end if;
...
begin
  ...
  if exists (select 1 from public.meeting_plans p where p.id = p_plan_id and p.confirmed) then
    raise exception 'PLAN_CONFIRMED' using errcode = 'P0001', message = '이미 확정된 약속이에요';
  end if;
  delete from public.meeting_plan_votes v
