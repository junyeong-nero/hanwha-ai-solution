...
- **배포 완료 (2026-09-03):** Supabase 프로젝트 연결, 마이그레이션 2개·시드 적용, Edge Function 5개 배포, Pages 백엔드 모드 전환. 브라우저 에뮬레이션으로 사번 로그인·두 사용자 채팅·LLM 매칭·AI 약속·만남 완료·행성 점등·새로고침 복원 검증. **남은 것:** OpenRouter 크레딧 충전 후 유료 모델로 확정(무료 모델은 연속 호출 시 429), 발표용 입장 코드 생성, 실기기 회귀.
- **Task 8(파일럿):** 제출 후 진행.
