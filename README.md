...
`assets/`에는 제출용 3개 파일을 모았습니다. 영상은 3분 5초 자막판이며, 발표 HTML은 브라우저에서 열면 됩니다. 보고서는 제출 페이지의 필수·선택 항목에 맞춰 작성했습니다. 현재 영상 제작 프로젝트는 [`video/`](video/README.md)에 있고, 이전 영상 묶음은 로컬 `outputs/demo-video/`, 중간 산출물은 `outputs/submission-video/`에 보존하고 Git에서 제외합니다.
...
| **채팅** | 모임별 익명 채팅방. **AI 장소 추천**으로 후보별 이유·주소·지도를 비교하고, 마음에 드는 곳을 채팅에서 이야기합니다 |
| **프로필** | 계열사·선호 지역·나이·관심사·취미와 매칭 성향. 변경 후 저장하면 매칭 정렬에 반영됩니다 |

핵심 흐름은 `매칭에서 참가 → 채팅방 → AI 장소 후보 추천 → 채팅으로 의견 나누기 → 만남 완료 → 베일 벗기기(익명이 실명으로) → 홈 행성 점등`입니다.
...
| AI | 모임·장소 추천은 **OpenAI Responses API · gpt-5.4-mini** (아래 참고) |
...
### 2. 장소 후보 추천 — 검색된 장소와 대화 맥락 결합

`suggest-meeting-plan`은 먼저 GPT가 최근 대화에서 음식·활동·지역 의도를 추출하고, 그 키워드로 카카오를 검색한 뒤 같은 대화와 실제 후보를 GPT에 전달해 장소를 추천합니다. 러닝 모임에서도 닭발을 먹자는 대화면 닭발 식당을 찾습니다. 시간·장소를 여기서 확정하지 않으며, 모든 현재 멤버가 후보와 이유를 추천받을 수 있습니다. 발신자 ID는 `참가자N`으로 치환하고 호출자 참가 이후 대화만 사용합니다(최대 30개·각 300자). 대화 본문에 직접 적힌 개인정보를 자동 제거하지는 않습니다.

- **장소 검증** — 후보를 검색 결과와 대조하고 주소·좌표·링크는 검색 값으로 채웁니다. 검색 결과가 없으면 최종 추천 호출을 생략하고 다시 시도를 안내합니다.
