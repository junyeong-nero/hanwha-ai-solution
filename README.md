...
- 계열사별 연결 현황을 궤도를 도는 행성으로 보여줍니다. 궤도마다 회전 속도와 시작 위치가 다릅니다.
- 은하계에는 **아는 사람이 생긴 계열사와 내 계열사**만 떠 있고, 새로운 계열사 동료와 만남이 성사될 때마다 그 계열사가 합류합니다.
...
- 추천한 장소 이름은 그대로 쓰지 않고 **카카오 장소 검색으로 검증**합니다. 실재하는 후보만 남겨 주소·좌표·상세 링크를 채우고, **지도와 목록에서 함께 비교**해 만남 장소를 고를 수 있습니다(목록 ↔ Marker 양방향 선택, 선택은 방 전체에 실시간 반영).
