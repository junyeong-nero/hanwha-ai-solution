-- 실행 로그와 별개로 외부 호출 전에 예산을 예약한다. 실패한 호출도 예산을 소비한다.
create table public.ai_request_budgets (
  scope text primary key,
  used integer not null check (used >= 0),
  expires_at timestamptz not null
);
alter table public.ai_request_budgets enable row level security;
revoke all on public.ai_request_budgets from anon, authenticated;
...
declare
  ...
  i integer;
begin
  if p_user is null or p_function is null or p_function not in ('recommend-meetings','suggest-meeting-plan')
    or (p_function = 'suggest-meeting-plan' and p_meeting is null)
  ...
  if p_function = 'suggest-meeting-plan' then
    v_scopes := array_append(v_scopes, 'meeting:' || p_meeting);
